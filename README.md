...
**Возможности:**
- 📁 Библиотека документов: загрузка, просмотр содержимого и извлечённых данных,
  скачивание оригинала, удаление; фильтры по категории и поиск по названию
...
| Слой | Реализация |
|---|---|
| Ingestion | Docling (локально) / LlamaParse (опц.) → Markdown; извлечение сущностей и платежей (claude-haiku-4-5), включая медицинские (врач/диагноз/лекарство/клиника); автокатегоризация: кредит / ЖКХ / мед / собственность |
