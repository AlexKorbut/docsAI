...
- 💬 Чат с проверенными ответами (Web UI + Telegram), фильтры по категории
  и члену семьи
- ⚙️ Фоновая обработка: загрузка возвращается мгновенно, распознавание идёт в фоне
  с прогрессом «распознано X из Y страниц» (`GET /api/jobs/{id}`)
- ✏️ Исправление распознанного: текст документа, название, категорию можно
  отредактировать — суммы, даты и поиск автоматически пересчитаются
